...
var edgeScreen : int = 50;
//Variables for the window:
private var windowSize:Vector2 = Vector2(Screen.width-edgeScreen, Screen.height-edgeScreen); //The size of the Inventory window.
...
private var itemIconSize : Vector2 = Vector2((Screen.width-edgeScreen)/6, (Screen.height-edgeScreen)/6); //The size of the item icons.
...
private var Offset : Vector2 = Vector2 ((32.0/2492)*(Screen.width-edgeScreen), (167.0/1080)*(Screen.height-edgeScreen)); //This will leave so many pixels between the edge of the window (x = horizontal and y = vertical).
...
function Awake()
{
	if (useCustomPosition == false)
	{
		...
		windowRect=Rect(edgeScreen/2,edgeScreen/2,Screen.width-edgeScreen,Screen.height-edgeScreen);
	}
	else
	{
...
function DisplayInventoryWindow(windowID:int)
{
	...
	var currentY = 0+ Offset.y; //Im setting the start y position to 18 to give room for the title bar on the window.
